...
var MathServer = function (overrideOptions) {
    ...
    var getCredentials = function (clientID, next) {
        ...
        logClient(clientID, "Want to get credentials.");
    };
    ...
    var getSSHOptions = function(credentials){
       console.log("Have credentials: " + credentials.host);
    ...
    var spawnMathProgramInSecureContainer = function (clientID, next) {
        getCredentials(clientID, function (credentials) {
            logClient(clientID, "In spawn, have ip: " + credentials.host);
    ...
    var listen = function () {
        ...
        io.on('connection', function (socket) {
            ...
            uploader.on("start", function(event) {
               fileUploadBuffer = "";
               console.log('started upload for file ' + event.file.name);
            });

            uploader.on("progress", function(event) {
               //console.log('getting data' + event.buffer);
               fileUploadBuffer += event.buffer;
            });
            ...
            uploader.on("complete", function(event) {
               ...
               sftpConnector.on('ready', function(){
                  sftpConnector.sftp(function(err, sftp) {
                     console.log('Buffer: ' + fileUploadBuffer + " fn " + event.file.name);
                     var stream = sftp.createWriteStream(event.file.name);
                     stream.write(fileUploadBuffer.toString());
                     stream.end(function(){
                        sftpConnector.end();
                     });
                  });

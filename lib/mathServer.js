...
var MathServer = function (overrideOptions) {
    ...
    var spawnMathProgramInSecureContainer = function (clientID, next) {
        getCredentials(clientID, function (credentials) {
            ...
            var conn = new Client();
            clients[clientID].sshConnector = conn;
            console.log('Conn: ' + conn);
            conn.on('ready', function() {
              console.log('Client :: ready');
               conn.exec('M2',{pty: true}, function(err, stream) {
                if (err) throw err;
                //stream.write("2+2\n");
    ...
    var sendDataToClient = function (clientID) {
        return function (dataObject) {
            ...
            // console.log("Receiving dataObject: " + dataObject + " of type " + dataObject.typeof);
    ...
    var listen = function () {
        ...
        io.on('connection', function (socket) {
            ...
            socket.on('interrupt', function () {
                console.log('Interrupt received. ')// + clients[clientId].mathProgramInstance.signal);
                var stream = clients[clientId].mathProgramInstance;
                //console.log("Has type: " + stream.typeof());
                //for (var id in stream) {
                //   try {
                //     console.log("stream property: " + id + " - " + stream[id]);
                //   } catch (err) {

...
var MathServer = function (overrideOptions) {
    ...
    if (options.SECURE_CONTAINERS) {
        ...
        var ipCollection = require('./dummy_containers.js').manager();
    }
    ...
    var getSSHCommand = function (ip) {
        var sshCommand = "ssh -oStrictHostKeyChecking=no ";
        if (options.SSH_KEY_PATH) {
            sshCommand += " -i " + options.SSH_KEY_PATH + " ";
        }
        if (options.SECURE_CONTAINERS_USER_NAME) {
            sshCommand += options.SECURE_CONTAINERS_USER_NAME + "@";
        }
        sshCommand += ip + " " + options.MATH_PROGRAM_COMMAND;
    ...
    var spawnMathProgramInSecureContainer = function (clientID, next) {
        getIp(clientID, function (ip) {
            ...
            var sshCommand = getSSHCommand(ip);
    ...
    var listen = function () {
        ...
        io.on('connection', function (socket) {
            ...
            socket.on('upload', function () {
                logClient(clientId, "UploadTutorial event received for " + clientId);
                ...
                form.on('file', function (name, file) {
                    ...
                    form.on('end', saveUploadedFile(temporaryFilename, file.name, clientId));
                });
                form.on('error', function (error) {
                    logClient(clientId, 'received error in upload: ' + error);
                    clients[clientID].socket.emit('error', 'error in upload.');
                });
                form.parse(request);

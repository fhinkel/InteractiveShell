...
var MathServer = function (overrideOptions) {
    ...
    var emitImageUrlToClient = function (clientId, imagePath) {
        ...
        var sshConnector = ssh2();

        sshConnector.on('end', function () {
        });

        sshConnector.on('ready', function () {
            sshConnector.sftp(function (err, sftp) {
    ...
    var fsReaddirTutorialWrapper = function(path, next){
        var totalPath = "public-" + options.MATH_PROGRAM + "/" + path;
    ...
    var fsExistsTutorialWrapper = function(path, next){
        var totalPath = "public-" + options.MATH_PROGRAM + "/" + path;
    ...
    var getListOfTutorials = function (fs) {
        return function(request, response)
        {
            ...
            async.filter(folderList, fsExistsTutorialWrapper, function(existingFolders){
               async.concat(existingFolders, fsReaddirTutorialWrapper, function(err, files){
    ...
    var initializeServer = function () {
        ...
        app.use('/admin', stats)
            .use('/getListOfTutorials', getListOfTutorials(fs))
            .use(unhandled);
    };

...
var MathServer = function (overrideOptions) {
    ...
    var getSSHOptions = function (credentials) {
        ...
        sshOptions.push('export WWWBROWSER="'+ __dirname + '/../open-www-ssh"; ' + "export PATH=" + __dirname + "/../:$PATH; " + options.MATH_PROGRAM_COMMAND);
        return sshOptions;
    };
    ...
    var captureViewHelp = function (data) {
        var imagePath = data.match(/>>WWWBROWSER_START>>(.*)<<WWWBROWSER_END/);
        if (imagePath) {
            return imagePath[1];
        }
    };
    ...
    var uploadHelpPath = function(clientID, viewHelp){
    ...
    var sendDataToClient = function (clientID) {
        return function (data) {
            ...
            var viewHelp = captureViewHelp(data);
            if(viewHelp) {
               console.log("Teh user wantz teh halp. " + viewHelp);
               uploadHelpPath(clientID, viewHelp);
               return;
            }

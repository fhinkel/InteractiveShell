...
var MathServer = function (overrideOptions) {
        ...
        sshConnector.on('ready', function () {
            sshConnector.sftp(function (err, sftp) {
                sftp.fastGet(imagePath, "./public/images/" + fileName, function (error) {
                    if (error) {
                        console.log("Error while downloading image. PATH: " + imagePath + ", ERROR: " + error);
                    }
    ...
    var emitEventUrlToClient = function(clientID, eventType, data){
        if (isViewHelpEvent(eventType)) {
            ...
            emitImageUrlToClient(clientID, eventType);
        }
    ...
    var initializeServer = function () {
        ...
        var t = require('./getListOfTutorials.js').Tutorials(prefix, fs);
        ...
        app.use('/admin', stats)
            .use('/getListOfTutorials', t.getList)
            .use(unhandled);
    };

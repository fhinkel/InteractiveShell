...
var ssh_docker_manager = function (overrideResources, overrideHostConfig, overrideGuestCredentials) {
    ...
    overrideDefaultOptions(overrideGuestCredentials, guestCredentials);
    ...
    var removeCredentials = function(guestCredentials){
        ...
        var removalCommand = "sudo docker rm -f " + credentials.containerName;
    ...
    var getNewCredentials = function (next) {
        var currentCredentials = JSON.parse(JSON.stringify(guestCredentials));
        guestCredentials.port++;
        ...
        connectToHostAndExecCmd(sshdCheckCmd, function(stream){
            stream.on('data', function(dataObject){
                ...
                if(data == 0){
                    ...
                    credentials.lastActiveTime = Date.now();
                    currentContainerNo++;
                    next(credentials);
                }
            });
    ...
    var updateLastActiveTime = function(credentials){
        credentials.lastActiveTime = Date.now();
    };

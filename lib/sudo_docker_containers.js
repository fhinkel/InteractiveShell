var sudo_docker_manager = function () {
    ...
    var getNewCredentials = function (next) {
      ...
      exec('sudo docker run -d -c ' + resources.cpus + ' -m ' + resources.memory + 'm --name m2Port' + currentCredentials.port + ' -p ' + currentCredentials.port + ':22 m2container', function (error, stdout, stderr) {

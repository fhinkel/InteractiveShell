var sudo_docker_manager = function () {
    ...
    var getNewCredentials = function (next) {
      ...
      exec(cmd, function (error, stdout, stderr) {
         ...
         waitForSSHD(next, currentCredentials);
      });
    };
   
    var waitForSSHD = function(next, credentials){
      var exec = require('child_process').exec;
      exec("ps aux | grep \"/usr/bin/sshd -D\" | wc -l", function(error, stdout, stderr){
         if(error){

...
\M2. You can type into it, as well as use the arrow keys for
...
\M2 code that are run by clicking on them. {\it Input} shows a
terminal window in which the user can type \M2 commands and execute them.
...
right hand side together with the output from \M2 to those
results.
...
The Node.js server functions as a bridge between \M2 instances and end user.
